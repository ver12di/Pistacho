...
// **NEW**: Supports pinned sorting for public view
// **FIX**: Correct DELETE logic to handle R2 errors gracefully
// ---------------------------------------------------
...
async function getRoleFromDatabase(db, userInfo, source = "unknown") {
    ...
    // console.log(`[getRoleFromDatabase @ ${source}] Inputs: userId=${userId}, email=${email}, nickname=${nickname}`); // Reduced logging
    ...
    try {
        ...
        if (userRecord) {
            // console.log(`[getRoleFromDatabase @ ${source}] Found user by userId. DB Role: ${userRecord.role}, DB Nickname: ${userRecord.dbNickname}, DB Email: ${userRecord.dbEmail}`); // Reduced logging
            ...
            return userRecord.role; // Return existing role
             ...
             // console.log(`[getRoleFromDatabase @ ${source}] User not found by userId. Trying by email...`); // Reduced logging
            ...
            if (email) {
                 ...
                 if (userRecordEmail) {
                     // console.log(`[getRoleFromDatabase @ ${source}] Found user by email. DB userId: ${userRecordEmail.dbUserId}, DB Role: ${userRecordEmail.role}, DB Nickname: ${userRecordEmail.dbNickname}`); // Reduced logging
            ...
            // Step 3: Create new user
             // console.log(`[getRoleFromDatabase @ ${source}] User not found by email either. Creating new user...`); // Reduced logging
...
export async function onRequestGet(context) {
    // ... (No changes needed) ...
     ...
     try {
         let stmt; let userInfo = await validateTokenAndGetUser(request, env); const currentUserRole = userInfo?.db_role ?? 'guest'; console.log(`[GET /api/ratings] User validated. Role: ${currentUserRole}, UserInfo:`, userInfo ? {sub: userInfo.sub, email: userInfo.email, role: userInfo.db_role} : null);
         ...
         console.log(`[GET /api/ratings] Executing list query...`); const { results } = await stmt.all(); console.log(`[GET /api/ratings] Found ${results.length} ratings in list view.`);
         ...
         console.log(`[GET /api/ratings] Returning ${parsedResults.length} parsed ratings.`); return new Response(JSON.stringify(parsedResults), { headers: { 'Content-Type': 'application/json' } });
...
export async function onRequestDelete(context) {
    ...
    try {
        ...
        if (imageKey && env.PISTACHO_BUCKET) {
            ...
            try {
                ...
                console.log(`[DELETE /api/ratings] Successfully deleted image key ${imageKey} from R2.`);
                 ...
                 // **Log R2 error but DO NOT throw** - proceed to delete from D1
        ...
        // **STEP 3**: Check D1 result and return success (200) or failure (404/500)
        if (result.changes > 0) {
            console.log(`[DELETE /api/ratings] Successfully deleted ID ${ratingId} from D1.`);
            // **FIX**: Return 200 OK here
            return new Response(JSON.stringify({ success: true, id: ratingId }), {
                status: 200,
                headers: { 'Content-Type': 'application/json'}
             });
        } else {
             // This case means the rating was already gone from D1
             console.log(`[DELETE /api/ratings] Deletion from D1 failed (changes=0). Rating might have been deleted already.`);
             // Throw error here to be caught below and return 404/500
            throw new Error("Deletion failed, rating might have been deleted already.");
        }
    } catch (e) { // Catches errors from auth, D1 checks, D1 delete, or JSON parsing
        ...
        else if (e.message.includes("not found") || e.message.includes("deleted already")) statusCode = 404;
        else if (e.message.includes("Missing ratingId")) statusCode = 400; // Bad request
