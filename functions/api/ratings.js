...
export async function onRequestGet(context) {
    ...
    try {
        ...
        const selectFields = `r.id, r.userId, r.userEmail, r.userNickname, r.timestamp,
                              ...
                              r.isPinned, r.fullData`; // <-- Added isPinned

        // Determine ORDER BY clause
        // **ORDER BY isPinned DESC for public/admin views**
        const defaultOrderBy = "ORDER BY r.timestamp DESC";
        const pinnedOrderBy = "ORDER BY r.isPinned DESC, r.timestamp DESC"; // Pinned first, then by time


        if (singleRatingId) {
            // ... (fetch single rating logic - no change needed for order)
             ...
             try { if (result.fullData && typeof result.fullData === 'string') { result.fullData = JSON.parse(result.fullData); } else { result.fullData = null; } }
            ...
            // Certified ratings probably don't need pinned sorting, just time
            stmt = env.DB.prepare(`SELECT ${selectFields} FROM ratings r WHERE r.isCertified = 1 ${defaultOrderBy}`);

        } else if (currentUserRole === 'admin' || currentUserRole === 'super_admin') {
            console.log(`[GET /api/ratings] Fetching all ratings for admin.`);
            // Admin history view: Pinned first might be useful
            stmt = env.DB.prepare(`SELECT ${selectFields} FROM ratings r ${pinnedOrderBy}`);

        } else if (userInfo) {
             console.log(`[GET /api/ratings] Fetching ratings for user ${userInfo.sub}.`);
             // User's own history: Time order makes most sense
             stmt = env.DB.prepare(`SELECT ${selectFields} FROM ratings r WHERE r.userId = ? ${defaultOrderBy}`).bind(userInfo.sub);
        } else {
             console.log(`[GET /api/ratings] Fetching all ratings for public view (guest).`);
             // Public community view: Pinned first
            stmt = env.DB.prepare(`SELECT ${selectFields} FROM ratings r ${pinnedOrderBy}`);
        }
        ...
        const parsedResults = results.map(row => {
            ...
            // Construct cigarInfo and finalGrade from top-level fields
            ...
            // **Convert isPinned from 0/1 to boolean**
            row.isPinned = !!row.isPinned; // Ensures it's true or false
...
export async function onRequestPost(context) {
    // ... (No changes needed here for pinning)
     const { request, env } = context;
    ...
    try {
        ...
        if (!ratingToSave || typeof ratingToSave !== 'object') { throw new Error("Invalid rating data received."); }
         if (!ratingToSave.config || !ratingToSave.ratings || ratingToSave.calculatedScore === undefined) { console.error("[POST /api/ratings] Error: Data to save is incomplete!", ratingToSave); throw new Error("Cannot save rating: Data is incomplete (missing config, ratings, or calculatedScore)."); }
        ...
        await env.DB.prepare(
          `INSERT INTO ratings (
            ...
            imageUrl, cigarReview, isPinned, fullData
           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)` // Added isPinned column
        ).bind(
          ...
          false, // Default isPinned to false
...
export async function onRequestPut(context) {
    ...
    try {
        ...
        if (!ratingToSave || typeof ratingToSave !== 'object') { throw new Error("Invalid rating data received."); }
         if (!ratingToSave.config || !ratingToSave.ratings || ratingToSave.calculatedScore === undefined) { console.error("[PUT /api/ratings] Error: Data to save is incomplete!", ratingToSave); throw new Error("Cannot save rating update: Data is incomplete (missing config, ratings, or calculatedScore)."); }
        // Security Check
        ...
        await env.DB.prepare(
          `UPDATE ratings SET
           ...
           WHERE id = ?` // isPinned is NOT updated here
        ).bind(
...
export async function onRequestDelete(context) {
    ...
    try {
        ...
        if (imageKey && env.PISTACHO_BUCKET) {
            ...
            try { await env.PISTACHO_BUCKET.delete(imageKey); console.log(`[DELETE /api/ratings] Successfully deleted image key ${imageKey} from R2.`); }
            catch (r2Err) { console.error(`[DELETE /api/ratings] Failed to delete R2 object ${imageKey}:`, r2Err); }
        } else { console.log(`[DELETE /api/ratings] No image key found or R2 bucket not configured. Skipping R2 delete.`); }
        if (result.changes > 0) { console.log(`[DELETE /api/ratings] Successfully deleted ID ${ratingId}`); return new Response(JSON.stringify({ success: true, id: ratingId }), { status: 200 }); }
        else { console.log(`[DELETE /api/ratings] Deletion from D1 failed (changes=0). Rating might have been deleted already.`); throw new Error("Deletion failed, rating might have been deleted already."); }
    } catch (e) { /* ... error handling ... */ console.error("[DELETE /api/ratings] Delete rating error:", e.message, e); let errorMessage = e.message || 'An unknown error occurred while deleting the rating.'; let statusCode = 500; if (e.message.includes('token') || e.message.includes('需要登录')) statusCode = 401; if (e.message.includes('Permission denied')) statusCode = 403; if (e.message.includes("not found")) statusCode = 404; return new Response(JSON.stringify({ error: errorMessage }), { status: statusCode, headers: { 'Content-Type': 'application/json' } }); }
}
